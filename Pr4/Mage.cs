...
public class Mage : Player
{
    ...
    public Mage()
    {
        ...
        MagicLevel = 1;
        GameLogic.OnPlayerInitialization += PlayerCreator;
    }
    ...
    public override void PlayerCreator()
    {
        ...
        do
        {
            ...
            {
                isMagicExists = CheckIfMagicExists(magic);
            }
    ...
    private bool CheckIfMagicExists(string magicToCheck)
    {
        foreach (var kvp in _spellsByMagic)
        {
            if (kvp.Key.ToString().ToLower() == magicToCheck.ToLower())
            {
                Magic = kvp.Key;
